class AICookApp {
    ...
    async generateRecipe() {
        if (!this.apiKey) {
            this.showError('Please save yoru Gemini API key first.');
            return;
        }
        ...
        try {
            ...
            console.log('Error generating receipe:', error);
            this.showError('Failed to generate receipe. Please check your API key and try again.');
        } finally {
    ...
    async callGeminiAPI(ingredients) {
     ...
     Make sure the recipe is practical and delicious!`;
        ...
        if (!response.ok) {
            ...
            throw new Error(`API Error: ${errorData.error?.message || 'Unknow error'}`);
        }
